...
class FeedData implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        feedTrackData.setArtist(env.getProperty("song.1.comment"));
        trackServices.saveUser(feedTrackData);

        feedTrackData.setId(Integer.parseInt(env.getProperty("song.2.id")));
        feedTrackData.setName(env.getProperty("song.2.name"));
        feedTrackData.setArtist(env.getProperty("song.2.comment"));
        trackServices.saveUser(feedTrackData);
